...
module.exports.run = async (bot, message, args, author) => {
  ...
  if (numParsedArgs > 10) return message.channel.send("That is too many! Try 10."); //25
  if (message.member.user.tag === author) {
    if (numParsedArgs === 1) {
      message.channel.bulkDelete(2);
      message.channel.send(`1 message cleared.`).then(msg => {setTimeout(() => msg.delete(), 5000)});
    } else {
      let numParsedArgsPLUSONE = numParsedArgs + 1;
      message.channel.bulkDelete(numParsedArgsPLUSONE);
      message.channel.send(`${numParsedArgs} messages cleared.`).then(msg => {setTimeout(() => msg.delete(), 5000)});
    }
  }
}
